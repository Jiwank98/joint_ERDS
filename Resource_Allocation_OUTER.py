...
import easy_sync_algorithm_INNER as sync_algorithm_main
import pickle
Algorithm = sync_algorithm_main.sync_algorithm
...
f_min = Algorithm(dimension, w_minset) #LBD : 최소 시간 구해주는 알고리즘에 모든 디지털 트윈 계산 컴퓨팅 자원을 최소한 주어 계산한 결과
f_max = Algorithm(dimension, w_maxset) # UBD : 양의 무한대로 두기 ,
print(f_min,f_max)
...
M_upperbound.append(Algorithm(dimension,initial_cal_M)) #upperbound set에 local upper bound 넣기
...
while True:
    ...
    setA_local_ubd = Algorithm(dimension, cal_setA)
    setB_local_ubd = Algorithm(dimension, cal_setB)
    feasible_flag = [0] * 2
    ...
    #이제 최종 결정된 setB를 바탕으로 x prime feasibility check, 가능하다면 UBD/LBD 업데이트하기
        ...
        if sum_resource > w_max: #infeasibility
            ...
            f_bar_min = Algorithm(dimension,z)
            ...
            for i in range(dimension):
                ...
                tmp_f_bar_max.append(Algorithm(dimension,local_setB))
    ...
    if feasible_flag[0]>0:
        ...
        if check_minimum(cal_setA, dimension, w_min) == True:
            Q.append(M_setA)
            M_upperbound.append(Algorithm(dimension,cal_setA))
    ...
    print("EPOCH={}, Now upper bound = {}, last Q = {}, last M_upper_bound = {} ".format(brb_iter,f_max,Q[-1],M_upperbound[-1]))
